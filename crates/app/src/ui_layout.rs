use kama_ui::{Align, Rect, ScrollState, Size};
...
impl Item {
    pub(crate) const fn new(width: Size, height: Size) -> Self {
        Self { width, height }
    }
    ...
    pub(crate) const fn height(height: f32) -> Self {
        Self::new(Size::Fill, Size::Pixels(height))
    }

    pub(crate) const fn width(width: f32) -> Self {
        Self::new(Size::Pixels(width), Size::Fill)
    }
}

pub(crate) fn centered(viewport: Rect, width: f32, height: f32) -> Rect {
...
pub(crate) fn inset(rect: Rect, padding: f32) -> Rect {
    column(rect, &[Item::fill()], 0.0, padding, Align::Start, None)[0]
}

pub(crate) fn row(rect: Rect, items: &[Item], gap: f32, padding: f32, align: Align) -> Vec<Rect> {
    flow(rect, items, true, gap, padding, align, None).1
}

pub(crate) fn row_scrolled(
    rect: Rect,
    items: &[Item],
    gap: f32,
    padding: f32,
...
pub(crate) fn scrolled_content(rect: Rect, offset: f32) -> Rect {
    column(
        rect,
...
pub(crate) fn stack(rect: Rect, start_y: f32, heights: &[f32]) -> Vec<Rect> {
    fit_column_at(
        rect,
        [rect.x, start_y],
        rect.width,
