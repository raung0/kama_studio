...
fn mapped_normal(input: VertexOutput) -> vec3<f32> {
    ...
    // Build the tangent frame from screen-space derivatives. This keeps imported meshes from
    // needing a second CPU-side tangent-generation/upload path just to use normal maps.
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    // Color textures are Rgba8UnormSrgb, so wgpu decodes RGB into linear light on sampling.
    ...
    // OBJ/FBX opacity maps are commonly grayscale in RGB; alpha-mask images commonly use A.
    // Prefer a nontrivial alpha channel, otherwise use the red channel. `min(r, a)` is wrong for
    // opaque grayscale maps because their RGB value is coverage while A stays 1.0.
    ...
    if (alpha_mode == 1u) {
        // glTF OPAQUE ignores base-color alpha.
        coverage = 1.0;
    } else if (alpha_mode == 2u) {
        // glTF MASK is a depth-writing cutout rather than a blended surface.
    ...
    // The editor has no refractive scene buffer here, so transmission is represented as coverage
    // in the premultiplied compositor. This preserves transparent glass instead of flattening it.
    ...
    // The editor's canonical GPU frame is premultiplied, linear-light RGBA.
