    ...
    pub(crate) fn editor_defaults() -> Self {
        ...
        for (id, label, description, shortcut, icon, command) in [
            (
                "project.new",
                "New Project",
                Some("Create an empty Kama project"),
                Some(KeyBinding::primary('n')),
                Some(AppIcon::New),
                EditorCommand::Action(PaletteAction::NewProject),
            ),
            (
                "composition.new",
                "New Composition",
                Some("Create a new composition"),
                Some(KeyBinding::primary_shift('n')),
                Some(AppIcon::Composition),
                EditorCommand::Action(PaletteAction::NewComposition),
            ),
            (
                "project.open",
                "Open Project",
                Some("Open a project"),
                Some(KeyBinding::primary('o')),
                Some(AppIcon::Open),
                EditorCommand::Action(PaletteAction::OpenProject),
            ),
            (
                "project.save",
                "Save Project",
                Some("Save current project"),
                Some(KeyBinding::primary('s')),
                Some(AppIcon::Save),
                EditorCommand::Action(PaletteAction::SaveProject),
            ),
            (
                "project.save-as",
                "Save Project As",
                Some("Save project to another file"),
                Some(KeyBinding::primary_shift('s')),
                Some(AppIcon::Save),
                EditorCommand::Action(PaletteAction::SaveProjectAs),
            ),
            (
                "media.import",
                "Import Media",
                Some("Add media"),
                Some(KeyBinding::primary('i')),
                Some(AppIcon::Import),
                EditorCommand::Action(PaletteAction::ImportMedia),
            ),
            (
                "workspace.reset-layout",
                "Reset Panel Layout",
                Some("Restore default editor workspace"),
                None,
                Some(AppIcon::Settings),
                EditorCommand::Action(PaletteAction::ResetLayout),
            ),
            (
                "workspace.toggle-current-panel-maximize",
                "Maximize / Restore Panel",
                Some("Toggle the focused panel between docked and maximized"),
                Some(KeyBinding::primary_shift('d')),
                Some(AppIcon::Maximize),
            ...
            (
                ...
                Some(AppIcon::Copy),
                EditorCommand::timeline(TimelineAction::CopySelection),
            ),
            (
                "edit.cut",
                "Cut",
                ...
                Some(AppIcon::Cut),
                EditorCommand::timeline(TimelineAction::CutSelection),
            ),
            (
                "edit.paste",
                "Paste",
                Some("Paste clips at playhead"),
                Some(KeyBinding::primary('v')),
                Some(AppIcon::Paste),
                EditorCommand::timeline(TimelineAction::Paste),
            ),
            (
                "timeline.power-duplicate",
                "Power Duplicate",
                Some("Duplicate selected clips and repeat the previous duplicate offset"),
                Some(KeyBinding::primary('j')),
                Some(AppIcon::Copy),
                EditorCommand::timeline(TimelineAction::PowerDuplicate),
            ),
            (
                "timeline.select-before-playhead",
                "Select Clips Before Playhead",
                Some("Select all clips before the playhead on the current track"),
                Some(KeyBinding::primary_shift_arrow_left()),
                Some(AppIcon::Timeline),
                EditorCommand::timeline(TimelineAction::SelectBeforePlayhead),
            ),
            (
                "timeline.select-after-playhead",
                "Select Clips After Playhead",
                Some("Select all clips after the playhead on the current track"),
                Some(KeyBinding::primary_shift_arrow_right()),
                Some(AppIcon::Timeline),
                EditorCommand::timeline(TimelineAction::SelectAfterPlayhead),
            ),
            (
                "timeline.delete-selection",
                "Delete Selection",
                Some("Delete selected timeline clips"),
                Some(KeyBinding::delete()),
                Some(AppIcon::Delete),
                EditorCommand::timeline(TimelineAction::DeleteSelection),
            ),
            (
                "timeline.group-selection",
                "Group Selection",
                Some("Group selected clips"),
                Some(KeyBinding::plain('g')),
                Some(AppIcon::Group),
                EditorCommand::timeline(TimelineAction::GroupSelection),
            ),
            (
                "timeline.ungroup-selection",
                "Ungroup Selection",
                Some("Remove selected clips from groups"),
                Some(KeyBinding::shifted('g')),
                Some(AppIcon::Ungroup),
                EditorCommand::timeline(TimelineAction::UngroupSelection),
            ),
            (
                "timeline.close-gap",
                "Close Gap",
                Some("Close gaps between selected clips"),
                None,
                Some(AppIcon::CloseGap),
                EditorCommand::timeline(TimelineAction::CloseGap),
            ),
            (
                "timeline.speed-duration",
                "Speed / Duration",
                Some("Change selected clip(s) speed"),
                None,
                Some(AppIcon::SpeedDuration),
                EditorCommand::timeline(TimelineAction::SpeedDuration),
            ),
            (
                "timeline.toggle-razor-tool",
                "Toggle Razor Tool",
                Some("Toggle the timeline razor tool"),
                Some(KeyBinding::plain('k')),
                Some(AppIcon::ClipCut),
                EditorCommand::timeline(TimelineAction::ToggleRazorTool),
            ),
            (
                "monitor.toggle-pen-tool",
                "Toggle Pen Tool",
                Some("Toggle monitor pen tool"),
                Some(KeyBinding::plain('p')),
                Some(AppIcon::Pen),
                EditorCommand::TogglePenTool,
            ),
            (
                "timeline.cut-at-playhead",
                "Cut at Playhead",
                Some(
                    "Split selected clips at the playhead, or all crossing clips when nothing is selected",
                ),
                Some(KeyBinding::primary('k')),
                Some(AppIcon::ClipCut),
                EditorCommand::timeline(TimelineAction::CutAtPlayhead),
            ),
            (
                "timeline.toggle-playback",
                "Play / Pause",
                Some("Toggle timeline playback"),
                Some(KeyBinding::space()),
                Some(AppIcon::Play),
                EditorCommand::timeline(TimelineAction::TogglePlayback),
            ),
            (
                "edit.undo",
                "Undo",
                Some("Move backward in edit history"),
            ...
            (
                "edit.redo",
                "Redo",
                Some("Move forward in edit history"),
                Some(KeyBinding::primary_shift('z')),
                Some(AppIcon::Redo),
                EditorCommand::redo(),
            ),
            (
            ...
            (
                ...
                "Settings",
                Some("Open application settings"),
                None,
                Some(AppIcon::Settings),
            ...
            (
                ...
                "Keybinds",
                Some("Configure keyboard shortcuts"),
                None,
                Some(AppIcon::Keybinds),
            ...
            (
                ...
                "Get help",
                Some("Open GitHub Q&A discussions"),
                None,
                None,
