...
fn effect(source: texture_2d<f32>, pixel: vec2<u32>, uv: vec2<f32>, output_size: vec2<u32>, threshold: f32, intensity: f32, radius: f32) -> vec4<f32> {
    ...
    // Keep every offset explicit. WebGPU's conservative WGSL validation on some backends rejects
    // dynamic indexing into function-local arrays even when the index is trivially bounded.
